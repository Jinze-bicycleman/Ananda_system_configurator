...
import { Step3VoltagePlatform } from "./step3-voltage-platform"
...
const labels = ["Sell Region & Regulation", "Bike Category", "Drive System", "Voltage Platform", "Motor Selection", "System Components", "Drivetrain", "Battery & Charging", "Accessories", "System Diagram", "Final Report"]

export function AnandaConfigurator() {
  const state = useAnandaStore()
  const [hydrated, setHydrated] = useState(false)
  useEffect(() => { setHydrated(true) }, [])
  const [open, setOpen] = useState(Math.min(Math.max(state.currentStep - 1, 0), 10))
  useEffect(() => { setOpen(Math.min(Math.max(state.currentStep - 1, 0), 10)) }, [state.currentStep])

  const complete = useMemo(() => [
    Boolean(state.sellRegion && state.regulation),
    Boolean(state.bikeCategory && state.wheelSize && state.tyreCircumferenceMm),
    hasDrive(state), hasVoltage(state), hasMotor(state), hasCoreComponents(state),
  ...
  const content = [
    ...
    <Step3VoltagePlatform key="voltage" />, <Step4MotorSelection key="motor" />, <Step5SystemComponents key="components" />,
