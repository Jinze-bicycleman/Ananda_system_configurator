...
export function Step1ProjectContext() {
  ...
  // Both accordion sections default open and stay open — selecting an
  // option no longer auto-collapses the section, and the two sections are
  // independent (not mutually exclusive) so a user can review Sell Region
  // and Regulation side by side.
  const [openSections, setOpenSections] = useState<{ sellRegion: boolean; regulation: boolean }>({ sellRegion: true, regulation: true })
  ...
  const toggle = (key: "sellRegion" | "regulation") => setOpenSections((prev) => ({ ...prev, [key]: !prev[key] }))
